...
const WatchlistView: React.FC<{ movies: Movie[] }> = ({ movies }) => {
  const [tab, setTab] = useState('Personal');
  const [isNewListModalOpen, setIsNewListModalOpen] = useState(false);
  const sharedMovies = movies.slice(0, 3); 
    ...
    <div className="py-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex justify-between items-center mb-10">
        ...
        <div className="flex bg-black/[0.03] p-1 rounded-2xl border border-main">
          {['Personal', `Shared (${sharedMovies.length})`].map(t => {
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {(tab === 'Personal' ? movies : sharedMovies).map(movie => (
