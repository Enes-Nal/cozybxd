...
const MovieGrid: React.FC<MovieGridProps> = ({ movies, onVote, onSchedule, onSelect, users }) => {
  return (
      ...
      {movies.map((movie, idx) => (
        ...
        >
          <div className="absolute top-3 right-3 z-10 flex flex-col gap-2 opacity-0 group-hover:opacity-100 transform translate-y-2 group-hover:translate-y-0 transition-all duration-300" onClick={(e) => e.stopPropagation()}>
            ...
            >
              <i className="fa-solid fa-heart text-[10px] text-[var(--accent-color)] group-hover/btn:scale-125 transition-transform"></i>
            </button>
            <button 
